...
export default function ShippingManagement() {
  ...
  return (
    <div className="space-y-6 text-slate-100">
      <div>
        <p className="text-sm font-semibold uppercase tracking-[0.25em] text-[#D4AF37]">Shipping</p>
        <h1 className="mt-2 text-3xl font-black">إدارة الشحن</h1>
      </div>
      ...
      <section className="rounded-2xl border border-white/10 bg-[#0F172A]/80 p-6">
        <div className="grid gap-4 md:grid-cols-6">
          ...
          <input value={form.city} onChange={(e) => setForm({ ...form, city: e.target.value })} placeholder="City or empty for default" className="rounded-xl border border-white/10 bg-black/30 px-4 py-3 outline-none focus:border-[#D4AF37]" />
          <input value={form.price} onChange={(e) => setForm({ ...form, price: e.target.value })} placeholder="Price" className="rounded-xl border border-white/10 bg-black/30 px-4 py-3 outline-none focus:border-[#D4AF37]" />
          <input value={form.freeShippingThreshold} onChange={(e) => setForm({ ...form, freeShippingThreshold: e.target.value })} placeholder="Free threshold" className="rounded-xl border border-white/10 bg-black/30 px-4 py-3 outline-none focus:border-[#D4AF37]" />
          ...
          <div key={rate.id} className="rounded-2xl border border-white/10 bg-[#0F172A]/80 p-5">
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className="text-sm text-slate-400">{rate.city || "Default all cities"}</p>
              </div>
              <button onClick={() => deleteRate.mutate({ id: rate.id })} className="rounded-lg border border-red-500/20 p-2 text-red-300">
                <Trash2 className="h-4 w-4" />
              </button>
            </div>
            <p className="mt-4 text-2xl font-black text-[#D4AF37]">{formatCurrency(rate.price, "en")}</p>
            <p className="mt-1 text-xs text-slate-400">Free from {rate.freeShippingThreshold || "not set"} EGP</p>
          </div>
        ))}
      </section>
    </div>
