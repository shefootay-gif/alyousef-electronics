...
export default function BrandLogo({
  ...
  const { siteName, logoUrl, content } = useTheme();
    ...
    <div className={`inline-flex items-center gap-3 ${className}`} aria-label={siteName}>
      {logoUrl ? (
        <img
          src={logoUrl}
          alt=""
          className={`${isMark ? "h-12 w-12" : "h-[52px] w-[52px]"} shrink-0 rounded-xl object-contain drop-shadow-[0_0_18px_rgba(212,175,55,0.35)]`}
        />
      ) : (
        <svg
          width={isMark ? 48 : 52}
