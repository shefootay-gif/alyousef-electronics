...
export default function Footer() {
  ...
  const { siteName, content } = useTheme();
    ...
    <footer className="bg-[#050505] pt-16 pb-8 relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-8">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-12">
          {/* Brand */}
          <div>
            <Link to="/" className="flex items-center gap-2 mb-4" aria-label={`${siteName} home`}>
              <BrandLogo showTagline />
            </Link>
            <p className="text-[#94A3B8] text-sm leading-relaxed mb-4">{pickLocalized(content.footerDescription, lang)}</p>
        ...
        <div className="border-t border-white/10 pt-8 flex flex-col md:flex-row items-center justify-between gap-4">
          <p className="text-[#64748B] text-sm">
            &copy; {new Date().getFullYear()} {siteName}. {lang === "ar" ? "جميع الحقوق محفوظة." : "All Rights Reserved."}
          </p>
          <div className="flex items-center gap-4">
            {contactLinks?.snapchat && (
              <a href={getSocialUrl(contactLinks.snapchat)} target="_blank" rel="noopener noreferrer" className="text-[#94A3B8] hover:text-[#D4AF37] transition-colors">
            ...
            {contactLinks?.twitter && (
              <a href={getSocialUrl(contactLinks.twitter)} target="_blank" rel="noopener noreferrer" className="text-[#94A3B8] hover:text-[#D4AF37] transition-colors">
                <span className="sr-only">X (Twitter)</span>
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
              </a>
            )}
            {contactLinks?.telegram && (
              <a href={getSocialUrl(contactLinks.telegram)} target="_blank" rel="noopener noreferrer" className="text-[#94A3B8] hover:text-[#D4AF37] transition-colors">
